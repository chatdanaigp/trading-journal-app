            ...
            <Card className="w-full max-w-md border-[#222] bg-[#0d0d0d]/90 backdrop-blur-xl text-white shadow-2xl shadow-black/50">
                <CardHeader className="space-y-3 text-center pb-2">
                    ...
                    <CardTitle className="text-2xl font-black tracking-tight">
                        {dict.verify.title.split(' ')[0]} <span className="text-[#ccf381]">{dict.verify.title.split(' ').slice(1).join(' ')}</span>
                    </CardTitle>
                    <CardDescription className="text-gray-400 text-sm leading-relaxed">
                        {dict.verify.subtitle}{' '}
                        <span className="text-[#ccf381] font-semibold">{dict.verify.brokerName}</span>{' '}
                        {dict.verify.subtitle2}
                    </CardDescription>
                </CardHeader>
                    ...
                    <form onSubmit={handleSubmit} className="space-y-5">
                        ...
                        <div className="space-y-2">
                            <label className="text-sm font-medium text-gray-300">
                                {dict.verify.inputLabel}
                            </label>
                            <Input
                                type="text"
                                inputMode="numeric"
                                placeholder={dict.verify.inputPlaceholder}
                                value={clientId}
                        ...
                        >
                            {loading ? (
                                ...
                                <>
                                    ...
                                    {dict.verify.success.split('!')[0] + '!'}
                                </>
                            ) : (
                                dict.verify.verifyBtn
                            )}
                        </Button>
